...
class UserCreateView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serialized_request_body.is_valid():
            ...
            data = {'user': new_user.first_name, 'link' : activation_link, 'to_email' : new_user.email}
            Utils.sendMail(data)
            return Response(serialized_request_body.data, status=200)
...
class Login(APIView):
    def post(self,request):
        ...
        try:
            ...
            return Response({'data': serializer.data, 'access_token': f'{access_token}', 'refresh_token':f'{refresh}'}, status=200)
